...
const TelegramChannel = () => {
  ...
  const handleSave = (channel) => {
    const { id, joinMessage, leaveMessage, marketingMessage, channelName } = channel;
    const payload = [
    ...
    axios.patch(`https://copartners.in:5134/api/TelegramMessage?Id=${id}`, payload)
      .then(response => {
  ...
  return (
    <div className="pb-[5rem] xl:pl-[12rem] md:pl-[10rem] pl-6 md:py-[6rem] pt-[8rem] bg-gradient min-h-screen">
      ...
      <div className="flex md:mt-[4rem] mt-[3rem] md:flex-row flex-col md:gap-6 gap-3 items-center justify-center md:ml-0 ml-[-2rem]">
        {channelData.map((channel, index) => (
          <div key={channel.id} className="border-2 border-[#202F49] rounded-[30px] md:w-[527px] w-[360px] md:h-[585px] flex flex-col gap-4 p-6">
            ...
            <div className="flex flex-col gap-4">
              <div className="relative">
                <label className="flex items-center justify-center bg-[#282F3E] text-white opacity-[50%]
                  w-[100px] h-[26px] rounded-[8px] font-[400] md:text-[14px] text-[13px] md:leading-[16px] leading-[13px] text-center">
                  Join Message
                </label>
                <textarea
                  typeof="text"
                  onChange={(e) => handleInputChange(index, 'joinMessage', e.target.value)}
              ...
              <div className="relative">
                ...
                <textarea
                  typeof="text"
                  onChange={(e) => handleInputChange(index, 'leaveMessage', e.target.value)}
              ...
              <div className="relative">
                ...
                <textarea
                  typeof="text"
                  onChange={(e) => handleInputChange(index, 'marketingMessage', e.target.value)}
              ...
              <div className="flex items-center justify-center bg-white border border-white rounded-[8px] w-[180px] h-[40px]">
                <button className="text-[14px]" onClick={() => handleSave(channel)}>
                  Save
                </button>
              </div>
